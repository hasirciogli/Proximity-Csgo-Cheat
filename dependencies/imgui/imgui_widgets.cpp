...
bool ImGui::CheckboxTypeB(const char* label, bool* v)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    window->DrawList->AddRectFilled(newRect.Min, newRect.Max, ImColor(43, 43, 43), 10);


    ImU32 check_col = GetColorU32(ImGuiCol_CheckMark);

    ImVec2 fUpperLeft(check_bb.GetTL().x, check_bb.GetTL().y + 2.f);
    ImVec2 fLowerRight(check_bb.GetCenter().x, check_bb.GetBR().y + 2.f);
    ImRect ffirst_pos(fUpperLeft, fLowerRight);

    //Setup new rect for mid square + circle
    ImVec2 sUpperLeft(check_bb.GetCenter().x, check_bb.GetTL().y + 2.f);
    ImVec2 sLowerRight(check_bb.GetBR().x, check_bb.GetBR().y + 2.f);
    ImRect ssecond_pos(sUpperLeft, sLowerRight);

    /*float _2_xsx_min = ssecond_pos.Min.x;
    float _2_xsx_max = ssecond_pos.Max.x;

    float _1_xsx_min = ffirst_pos.Min.x;
    float _1_xsx_max = ffirst_pos.Max.x;*/
    

    if (*v) {
        /*_2_xsx_min = _1_xsx_min;
        _2_xsx_max = _1_xsx_max;


        _1_xsx_min = TKlerp(_1_xsx_min, ssecond_pos.Min.x, GetIO().DeltaTime * 6);
        _1_xsx_max = TKlerp(_1_xsx_max, ssecond_pos.Max.x, GetIO().DeltaTime * 6);

        ImRect ttarget(sUpperLeft, sLowerRight); 

        ttarget.Min.x = _1_xsx_min;
        ttarget.Max.x = _1_xsx_max;*/

        window->DrawList->AddRectFilled(ssecond_pos.Min, ssecond_pos.Max, ImColor(20, 130, 20), 10);
    }
    else
    {
        /*_1_xsx_min = _2_xsx_min;
        _1_xsx_max = _2_xsx_max;
        
        _2_xsx_min = TKlerp(_2_xsx_min, ffirst_pos.Min.x, GetIO().DeltaTime * 6);
        _2_xsx_max = TKlerp(_2_xsx_max, ffirst_pos.Max.x, GetIO().DeltaTime * 6);

        ImRect ttarget(fUpperLeft, fLowerRight);

        ttarget.Min.x = _2_xsx_min;
        ttarget.Max.x = _2_xsx_max;*/

        window->DrawList->AddRectFilled(ffirst_pos.Min, ffirst_pos.Max, ImColor(60, 60, 60), 10);
    }
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    window->DrawList->AddCircleFilled(ImVec2(grab_bb.Min.x + (grab_bb.GetWidth() / 2), grab_bb.Min.y + 1), 6.0f, ImColor(1.0f, 1.0f, 1.0f), 30);
...
bool ImGui::BeginListBox(const char* label, const ImVec2& size_arg)
{
    ...
    const ImVec2 label_size = ImVec2(0, 0); //CalcTextSize(label, NULL, true);
